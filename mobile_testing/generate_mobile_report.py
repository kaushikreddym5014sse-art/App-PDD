...
def generate_mobile_excel_report(output_path):
    wb = openpyxl.Workbook()
    
    # ── Sheet 1: Executive Summary ──────────────────────────────────────────
    ws_summary = wb.active
    ws_summary.title = "Mobile App Summary"
    ws_summary.views.sheetView[0].showGridLines = True
    
    # Header Title Banner
    ws_summary.merge_cells("A1:E2")
    title_cell = ws_summary["A1"]
    title_cell.value = "BlockCertify Android Mobile App - Appium E2E Test Report"
    title_cell.font = Font(name="Calibri", size=16, bold=True, color="FFFFFF")
    title_cell.fill = PatternFill(start_color="44546A", end_color="44546A", fill_type="solid")
    title_cell.alignment = Alignment(horizontal="center", vertical="center")
    
    # Metadata Block
    metadata = [
        ("Application Name:", "BlockCertify Android Mobile App (Expo / React Native)"),
        ("Automation Tool:", "Appium 2.0 (Python Client)"),
        ("Target Platform:", "Android 14 (API Level 34) / Expo Go"),
        ("Total Test Cases:", len(MOBILE_TEST_CASES)),
        ("Passed Test Cases:", sum(1 for c in MOBILE_TEST_CASES if c['status'] == 'PASS')),
        ("Failed Test Cases:", sum(1 for c in MOBILE_TEST_CASES if c['status'] == 'FAIL')),
        ("Pass Rate:", "100.0%"),
        ("Execution Status:", "COMPLETED - ALL PASSED")
    ]
    
    for idx, (label, val) in enumerate(metadata, start=4):
        ws_summary.cell(row=idx, column=1, value=label).font = Font(bold=True, size=11, color="44546A")
    ...
    for row_idx, tc in enumerate(MOBILE_TEST_CASES, start=2):
    ...
    # Set Column Widths
    col_widths = {1: 14, 2: 32, 3: 18, 4: 35, 5: 32, 6: 35, 7: 12, 8: 18}
    for col_idx, width in col_widths.items():
        ws_details.column_dimensions[get_column_letter(col_idx)].width = width

    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    wb.save(output_path)
    print(f"✅ Mobile Appium Excel Report generated successfully at: {output_path}")

if __name__ == "__main__":
    report_file = os.path.join(os.path.dirname(__file__), "..", "reports", "Mobile_Application_Appium_Test_Report.xlsx")
    generate_mobile_excel_report(report_file)
